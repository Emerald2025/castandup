	...
	if ( $attendee_pre_approved ) {
		...
		if ( $total_cost  > 0 ) {
			...
			if ($org_options['email_before_payment'] == 'Y') {
				event_espresso_email_confirmations(array('session_id' => $session_id, 'send_admin_email' => 'true', 'send_attendee_email' => 'false'));
			}
...
function event_espresso_pay() {
	...
	if ( $REG_ID != false && empty($payment_data['attendee_id'] )) {
		...
		/**
		 * For new invoice payments we need to send an invoice confirmation email with:
		 * - a reminder to pay
		 * - a link to the invoice
		 */
		if (isset($_GET['payment_type']) && $_GET['payment_type'] == 'invoice') {
