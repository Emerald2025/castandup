...
function espresso_prepare_admin_email($data) {
	...
	// Show event status (e.g. registration closed, event passed, etc.)
	$event_status = event_espresso_get_is_active($data->event->id);
	if (isset($event_status['status'])) {
		$admin_additional_info .= '<strong>' . __('Event Status: ', 'event_espresso') . '</strong><br />';
		$admin_additional_info .= $event_status['status'] . '<br/><br/>';
	}

	// Show payment status (e.g. incomplete, )
	if (!empty($data->attendee->payment_status)) {
		$admin_additional_info .= '<strong>' . __('Payment Status: ', 'event_espresso') . '</strong><br />';
		$admin_additional_info .= $data->attendee->payment_status . '<br/><br/>';
	}
